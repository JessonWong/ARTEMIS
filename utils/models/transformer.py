...
class Transformer(nn.Module):
    ...
    def forward(self, style, mask , content, pos_embed_c, pos_embed_s):
        ...
        style = style.flatten(2).permute(2, 0, 1)   # (16,bs,512)
        if pos_embed_s is not None:
            pos_embed_s = pos_embed_s.flatten(2).permute(2, 0, 1)
      
        content = content.flatten(2).permute(2, 0, 1)   # (16,bs,512)
        if pos_embed_c is not None:
            pos_embed_c = pos_embed_c.flatten(2).permute(2, 0, 1)

        
        style = self.encoder_s(style, src_key_padding_mask=mask, pos=pos_embed_s)       # (16,bs,512)
        content = self.encoder_c(content, src_key_padding_mask=mask, pos=pos_embed_c)   # (16,bs,512)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        # tgt:目标序列，tgt_mask:掩码，memory:编码器的输出
        output = tgt    #
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        q = self.with_pos_embed(tgt, query_pos) # (16,bs,512)
        k = self.with_pos_embed(memory, pos)    # (16,1,512)
        v = memory  # (16,1,512)
