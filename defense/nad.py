...
class NADModelTrainer(PureCleanModelTrainer):
    ...
    def train_epoch(self, args, trainloader, nets, optimizer, scheduler, criterions, epoch):
        ...
        for idx, (inputs, labels, original_index, poison_indicator, original_targets) in enumerate(trainloader):
            inputs, labels = inputs.to(args.device, non_blocking=self.non_blocking), labels.to(args.device,
        ...
        logging.info(
            f'Epoch{epoch}: Loss:{train_loss} Training Acc:{avg_acc_clean}({total_clean_correct}/{total_clean})')
...
class nad(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--beta1', type=int, help='beta of low layer', default=0.5)
        parser.add_argument('--beta2', type=int, help='beta of middle layer', default=0.5)
        parser.add_argument('--beta3', type=int, help='beta of high layer', default=0.5)
