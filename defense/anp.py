...
class anp(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--save_path', type=str, help='the location of save')
        ...
        parser.add_argument('--pruning_by', type=str, choices=['number', 'threshold'], default='threshold')
        parser.add_argument('--pruning_max', type=float, help='the maximum number/threshold for pruning', default=0.90)
        parser.add_argument('--pruning_step', type=float, help='the step size for evaluating the pruning', default=0.05)

        parser.add_argument('--pruning_number', type=float, help='the default number/threshold for pruning', default=None)
    ...
    def evaluate_by_number(self, args, model, mask_values, pruning_max, pruning_step, criterion,test_dataloader_dict, best_asr, acc_ori, save = True):
        ...
        for start in range(0, nb_max + 1, nb_step):
            ...
            print(f"acc ratio now: {abs(test_acc - acc_ori)/acc_ori}")
    ...
    def mitigation(self):
        ...
        for i in range(nb_repeat):
            ...
            train_loss, train_acc = mask_train(args, model=net, criterion=criterion, data_loader=clean_val_loader,
                                            mask_opt=mask_optimizer, noise_opt=noise_optimizer) # train
            cl_test_loss, cl_test_acc = test(args, model=net, criterion=criterion, data_loader=clean_test_loader)   # test clean
            po_test_loss, po_test_acc = test(args, model=net, criterion=criterion, data_loader=poison_test_loader)  # test bd
            end = time.time()
            logging.info('{} \t {:.3f} \t {:.1f} \t {:.4f} \t TrainACC:{:.4f} \t {:.4f} \t ASR:{:.4f} \t {:.4f} \t CleanACC:{:.4f}'.format(
...
if __name__ == '__main__':
    ...
    if "result_file" not in args.__dict__:
        ...
        args.result_file = 'defense_test_badnet'
    result = anp_method.defense(args)