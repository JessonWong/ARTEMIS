...
class NADModelTrainer(PureCleanModelTrainer):
    def __init__(self, model, teacher_model, criterions):
        super(NADModelTrainer, self).__init__(model)    # model: student net
    ...
    def train_with_test_each_epoch_on_mix(self,
        ...
        for epoch in range(total_epoch_num):
            ...
            print(f" epoch: {epoch} / {total_epoch_num}")
            nets = {
                'student':self.model,
    ...
    def train_epoch(self,args,trainloader,nets,optimizer,scheduler,criterions,epoch):
        ...
        for idx, (inputs, labels, original_index, poison_indicator, original_targets) in enumerate(trainloader):
            ...
            if args.model == 'preactresnet18':  # sage 的防御，只用对student进行训练，无需teacher的参与。
                outputs_s = snet(inputs)
                act1, act2, act3, act4, outputs_s = snet.lossforward_for_sage(inputs)
                ...
                at3_loss = criterionAT(act4, act3).detach() * args.beta3
                at2_loss = criterionAT(act3, act2).detach() * args.beta2
                at1_loss = criterionAT(act2, act1).detach() * args.beta1
...
class nad(defense):
    ...
    def add_arguments(parser):
        ...
        parser.add_argument('--save_path', type=str, help='the location of save dir')
        ...
        parser.add_argument('--yaml_path', type=str, default="./config/defense/nad/config.yaml",
        ...
        parser.add_argument('--ratio', type=float, help='ratio of training data', default=0.4)
        parser.add_argument('--beta1', type=int, help='beta of low layer', default=1000)
        parser.add_argument('--beta2', type=int, help='beta of middle layer', default=500)
        parser.add_argument('--beta3', type=int, help='beta of high layer', default=1000)
        parser.add_argument('--p', type=float, help='power for AT', default=2)
    ...
    def mitigation(self):
        ...
        print(f"args.model: {args.model}")  # preactresnet18
        ...
        criterionAT = AT(args.p)    # p 指代的是 p次幂
...
if __name__ == '__main__':
    ...
    if "result_file" not in args.__dict__:
        ...
        args.result_file = 'defense_test_badnet'
    result = ft_method.defense(args)

# bdmodel 作为stu模型，干净的model作为teacher model